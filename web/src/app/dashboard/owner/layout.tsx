...
import { Car, ShieldCheck, LayoutDashboard, LogOut, ChevronRight } from 'lucide-react';
...
export default function OwnerLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    { name: 'Tableau de Bord', href: '/dashboard/owner', icon: LayoutDashboard },
    { name: 'Mes Véhicules', href: '/dashboard/owner/vehicles', icon: Car },
    { name: 'Vérification', href: '/dashboard/owner/verification', icon: ShieldCheck },
  ];
    ...
    <div className={`min-h-screen flex ${theme.bg} ${theme.text}`}>
      ...
      <aside className={`w-64 border-r ${theme.border} border flex flex-col transition-all duration-300 ${theme.cardBg}`}>
        <div className="p-6 mb-8">
          ...
          <p className="text-[10px] uppercase tracking-widest opacity-50 mt-1">Espace Propriétaire</p>
        </div>
